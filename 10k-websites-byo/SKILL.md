...
**If other website skills are installed, this one governs.** A generation vendor's own site builder, or anything picked up in an earlier project, may sit alongside this skill. The user asked for this skill, so the whole build runs on this skill alone: its phases, its laws, its gates, its standards. Never blend another skill's website workflow into this one, and never hand the build off to one.

**No generation tool is connected here, and that is the design.** This build never calls an image or video generator itself. The user generates the hero assets in whatever tool they already have, and hands the files over. You write every prompt and name every setting, they run it, you inspect what comes back and gate it. The full handoff protocol is `references/asset-intake.md`. If a generation tool DOES happen to be available in the session, use it directly and keep every gate exactly as written; the handoff is the floor, not a ceiling.
...
Get the BUILD tools working and the generation route settled so the creative flow never stops later. Hosting is deliberately not set up here: nothing in the build needs it, so the hosting connector waits for Phase 10, where going live is the payoff instead of an early chore. No restart is planned in this phase; setup flows straight into the creative work in one unbroken session. Run it like an installer: scan, report, fix one thing at a time, verify, next.

Assume nothing about what is already set up. Some users arrive with ffmpeg, Node, and a paid video generator they use every week. Others arrive with a completely fresh machine and no generator at all. Both are normal, neither is a surprise, and the scan plus one question tells you which one this is. The steps below cover whatever is missing, in order, and nothing more.

1. **Scan the system before asking the user anything.** Check each prerequisite yourself: `ffmpeg` runs in the terminal? Node.js installed (`node --version`; the local preview server runs through it)? Does the project folder already hold usable footage or stills from an earlier session? Also note quietly whether hosting tools happen to be connected already from an earlier project. That note is for Phase 10 only, never for the report.
2. **Report the scan as a simple checklist.** A ✓/✗ line per item and the plan for the missing ones in order. The checklist covers the build tools plus one line for the generation route, which the scan cannot see and step 4 settles. If everything the build needs is present, say so, run step 4, give the honest-costs talk, and go straight to declaring setup complete.
...
4. **The generation route, the one thing only the user can answer.** Read `references/asset-intake.md` first, then settle three things with clickable questions, in this order:
   - **Which tool makes the video.** Any image-to-video tool the user already has works: the requirement is not a brand, it is four capabilities. It must take a starting image, output 16:9 at 1080p or better, export a clean mp4 with NO watermark, and let them download the file to their own machine. Offer the tools they are likely to have as options and let Other catch the rest. If they have no tool at all, say so plainly: most have a free tier that watermarks, and a watermarked hero sinks the whole site, so the honest choices are a paid tier, a genuinely watermark-free free tier, or existing real footage instead of generation. Never guess a price; ask what one video costs on their plan and write it down for the money talk.
   - **Which tool makes the stills** (the starting frame and two to four supporting images). Often a different tool from the video one, and that is fine. Same requirements minus the motion: 16:9, high resolution, no watermark, downloadable.
   - **How files come back.** Two routes, their pick: drag the file straight into this chat, or save it into the project's `_incoming/` folder and say it is there. Create `_incoming/` and `_review/` now, outside the deploy folder, and name the files they should use (`start-frame.png`, `hero-raw.mp4`).
   Then VERIFY, because "I have that tool" is a claim, not a check: ask them to send one file through the route they chose right now, any small image, before the pipeline depends on it. Read the file yourself and confirm it arrived. Once verified, give the honest-costs talk (see `references/deploy.md`).
5. **Declare setup complete** with the checklist all ✓, the generation route named, and the test file received. If the whole process ever needs an app restart, it happens in Phase 10 when hosting connects, and even there only sometimes.
...
The storyboard approval is the cheapest gate in the pipeline. One yes before the user spends a generation beats three re-rolls after.

## Phase 6: Generate the hero (gates included, money watched)

Read `references/prompt-laws.md` before writing any generation prompt, and `references/asset-intake.md` before handing anything over. You do not run the generator here. You write the prompt, name every setting, wait, then inspect what comes back. Every gate below still holds, unchanged.

1. **Set the money picture in their tool's units.** You cannot see their meter, so the numbers come from them: use the per-image and per-video prices they gave in Phase 1 and say the road ahead in one honest line, in their currency or credits. One starting frame, one video, then two to four supporting stills. The cheap step comes first on purpose: they see the brand's world before spending the big one. Restate the running total before each handoff, and never let a re-roll be a surprise.
2. **The starting frame** (one image): write the prompt from the start-frame template, then hand it over in the shape `asset-intake.md` specifies: the prompt in one copy-paste block, the settings named in plain words (16:9, highest resolution offered, no upscale, download the original), and nothing else in that message. A user's real product photo can be the starting frame instead, and then no generation happens at all.
3. **Inspect the image yourself before it gets animated.** Open the file and look at it. Check for sneaked-in trademarks and logos (AI loves to add real brand marks), broken anatomy, composition, and a watermark from a free tier. A bad frame is one cheap re-run now or a whole video wasted later. Then show the user: a quick yes here is cheap insurance.
4. **Confirm the video settings before any video is spent.** The tool was chosen in Phase 1, so this is not a re-ask. If their tool offers several models or quality modes at different prices, give the one honest line of tradeoff (the top tier buys the highest ceiling; a mid tier turns one shot into several, which changes the video gate from frightening into a normal creative decision) and let them pick with their own numbers in hand. Then name the exact settings for the run: image-to-video from the approved frame, 1080p, 16:9, about 6 seconds, no audio, no music, no upscale, no auto-enhance, download the original file.
5. **The video:** write the prompt from the video template and hand it over the same way. Then STOP and wait. They have left the chat to run it, and it takes minutes on their side plus their own inspection. Do not ask if it is ready. Use the wait as described below.
6. **Inspect the video yourself the moment it lands:** run the intake check in `asset-intake.md` (codec, resolution, aspect, duration, audio track, file size), then extract start, middle, and end frames with ffmpeg and examine them: anatomy, the transition if any, whether the ending truly rests, and any watermark. An intake failure goes back to them immediately with the one setting that was wrong, before the creative gate is even opened.
7. **⛔ THE VIDEO GATE (never skip, applies to EVERY segment in a chained journey):** save the video where the user can double-click it, in the `_review/` folder OUTSIDE the deploy folder, and have them watch it before the site is built around it. (Silent scaffolding during the wait is fine; nothing is shown or finished until this gate passes.) Offer your own honest critique alongside. Name what a re-roll costs them in their tool's units, so they decide with the numbers in hand. If they reject it, take their plain-words feedback, adjust the prompt or the starting frame, and hand over a re-run. If a concept fails three video attempts, stop iterating the prompt and change the concept: that is a concept problem, not a prompt problem.
8. **Supporting imagery (after the video passes the gate):** two to four stills for the lower sections, all in the SAME world as the approved hero footage: same palette, same lighting, same grade, described explicitly in each prompt. Hand all of them over in ONE message, numbered, so the user runs them in a single sitting instead of a stop-start relay. Every parallel element gets equal treatment: if a section has three steps, all three get images, because an asymmetry reads as a hole to a first-time visitor. Real businesses get their working product photos and logo used directly; real businesses with no photos get generated stills in the same declared world, honoring the disclosure decision made in Phase 2; invented brands get generated placeholders. Supplied assets that are already the product's true face (screenshots, renders, packaging art) go into the site directly, crisp and untouched; ask for generated stills only for the sections that have no real asset. Inspect every image yourself, then show the user the set before building with them.

**The brand-coherence inspection (every generated asset).** Details inside a generated image must agree with the brand's own story, not just look good. Name the brand's signature details before writing the prompt: its color, its mark, its materials, whatever the story is built on. Then inspect each image against that list, not just against trademarks and anatomy. A generated product shot can carry the category's classic detail in the wrong color, and when the brand's whole story is built on its own color, the target audience clocks it instantly. A cheap re-run fixes it now; a shipped miss undermines the brand.

**Inspection runs in both directions.** The steps above hunt for errors, but inspection also catches gifts. The generator sometimes improves on the storyboard: an unplanned shape, a better composition, a subtler detail than you designed. When the footage improves on the plan, flex the beat map and the layout to feature the gift instead of forcing the original plan. This includes the settle: if the ending frame's key element lands off-center, move the settle text to honor it. Law 7 works in both directions: the layout composes the footage before generation, and the delivered footage recomposes the layout after.

**The wait is long, and it belongs to the user.** Every generation here is a round trip through another application: they read the prompt, set it up, wait for the render, download the file, and come back. That is minutes at best, and a chained journey is that loop repeated per segment. Say when you hand off that you will be building the page in the meantime, then go quiet and work: scaffold the site, wire the sections, write nothing that depends on footage you have not seen. Never nudge, never poll, never ask if it is done. Their next message is the file.

## Phase 7: Process the assets

No generation here, and nothing costs anything. Follow `references/ffmpeg-recipes.md` exactly: the scrub re-encode with a short keyframe interval, the poster and ending frame, web-sized stills with one clean compression pass, segment concat for chained journeys, and keeping raw and review files OUT of the deploy folder so they never ship.
...
- `references/prompt-laws.md`: the twelve hero-video laws, prompt templates, the chaining recipe, preset declining, and the money talk when you cannot see the meter. Read before writing any generation prompt.
- `references/asset-intake.md`: the handoff protocol. How to hand a prompt over, what settings to name, how files come back, the intake check every file must pass, and what to do when an export is wrong. Read before Phase 1's step 4.
