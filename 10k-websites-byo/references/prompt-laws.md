...
Design every shot by these laws BEFORE generating. They predict which concepts land first-try and which burn the user's generations. Each one came from a real build that either succeeded because of it or failed without it.
...
### Start frame template (image, 16:9, highest resolution offered)
...
### Video template (image-to-video, 1080p, 16:9, about 6 seconds, no audio)
...
Ask for 1080p, not 4K. The web version gets re-encoded and compressed anyway, and 4K only multiplies the cost, the download time, and the chance the tool charges more for it.
...
3. Hand that PNG back to the user, saved in `_review/` and named for its segment (`seg-01-final.png`), and tell them in one line to load it as the starting image for the next segment. This is the bridge, and it is a file, not an API call: they upload it in their own tool the same way they uploaded the first start frame. Say plainly that it must be that exact PNG and not a screenshot of the video, because a screenshot loses the resolution the join depends on.
...
## Money, when you cannot see the meter

The generation runs in the user's own tool, so there is no free price check to call and no balance to read. That does not excuse the money talk; it moves it. Ask once in Phase 1 what one image and one video cost on their plan, and roughly what they have left. Write both numbers down and use them for the rest of the build.

Then state the total before every handoff, in their units, not yours: the starting frame's price before the frame, the video's price once the frame is approved, and the supporting stills folded into one number. A whole Tier 1 build is one image, one video, and two to four stills, plus whatever the re-rolls cost. For a chained journey, present the full total up front, per segment, because the video price multiplies by the segment count and that total is the real decision.

**The model or quality tier is a real choice, and the user makes it.** Most generators offer several models or quality modes at very different prices for the same shot, and the spread between a top tier and a mid tier is commonly several times over. Present the tradeoff honestly and let them pick: the top tier buys the highest ceiling on prompt fidelity and motion; a mid tier is legitimate and turns a small balance from one shot into many, which changes the video gate from frightening into a normal creative decision. Their money, their tool, their choice, made before anything is spent.

Two habits keep the spending honest without a meter. Never hand over a generation the user has not agreed to pay for in that same message. And when a re-roll is the right call, say what it costs them before recommending it, so a creative opinion never reads as free advice with their money attached.
