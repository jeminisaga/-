...
- The generation happens in the user's own tool, so its prices are theirs, not yours to guess. Get the per-image and per-video numbers from them in Phase 1 and quote those back for the rest of the build. Never invent a price, and never quote a number written in this file.
- A Tier 1 build spends one image, one video, and two to four supporting stills, plus re-rolls. A chained journey multiplies the video by the segment count, and that multiplication is the real budget decision. Present the whole total before the first handoff.
- Most generators offer several models or quality tiers at very different prices for the same shot. The tradeoff, and how to present it honestly, is in the money section of `prompt-laws.md`. The user picks.
- Free tiers usually watermark the export, and a watermarked hero sinks a site meant to look like it cost thousands. Say that plainly during setup, before anyone burns a free generation on a file the build cannot use.
- Everything else in this pipeline is free: ffmpeg, the local preview, the build itself, and every re-deploy.
