...
| Symptom | Cause | Fix |
|---|---|---|
| The returned video carries the tool's watermark | A free tier, or a watermark-free plan with the export toggle left on | Do not crop or blur it; a cropped hero breaks the composed framing and a blurred one is visible. Re-export from the same generation if the tool allows it, otherwise it is a plan problem and the user decides |
| The returned file is a link to the tool's page instead of a file | The user shared instead of downloading | Ask for the downloaded original: the page copy is often re-encoded, watermarked, or expiring |
| The returned video is 4K, or 9:16, or has an audio track | Settings left at the tool's defaults | 4K downscales cleanly in the scrub encode and audio strips with `-an`, so both are fixable locally. A 9:16 or 1:1 aspect is not croppable to a 16:9 hero without wrecking the composition: name the aspect setting and have them re-run |
| The second segment of a chain does not continue the first | The start image was a screenshot of the video instead of the extracted PNG | Hand over the ffmpeg-extracted full-quality PNG and have them upload that exact file |
...
| The user names a generation tool but no file ever arrives through the route they picked | The route was never tested, so a drag-and-drop limit or a folder path problem only shows at the worst moment | Send one test file through the route during Phase 1 setup, before the pipeline depends on it; a claim is not a check |
